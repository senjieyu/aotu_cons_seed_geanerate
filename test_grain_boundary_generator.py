...
It uses unittest to verify the functionality of generating grain boundaries
for various crystal systems (Cubic, Hexagonal, Tetragonal, Orthorhombic, Rhombohedral).

Prerequisites:
    - A folder named 'inputfile' in the same directory as this script.
    - The 'inputfile' folder must contain the required CIF files (e.g., Cu_mp-30_primitive.cif).
"""
...
# Define the directory containing the input CIF files
test_dir = os.path.join(os.path.dirname(__file__), "inputfile")
...
class Test_grain_boundary_generator(PymatgenTest):
    ...
    def setUpClass(cls):
        """
        Set up the test environment by loading structures from CIF files.
        These structures are used across multiple tests.
        """
        # Load Copper (Cu) primitive cell
        cls.Cu_prim = Structure.from_file(os.path.join(test_dir, "Cu_mp-30_primitive.cif"))
        cls.GB_Cu_prim = GBGenerator(cls.Cu_prim)
        
        # Load Copper (Cu) conventional cell
    ...
    def test_gb_from_parameters(self):
        """
        Test generating grain boundaries from specific parameters:
        - Rotation axis
        - Rotation angle (sigma value)
        - Expansion times
        - Vacuum thickness
        - Normal direction
        - GB plane
        """
        # Test 1: Generate GB from FCC primitive cell, axis [1,2,3], sigma 9
    ...
    def test_gb_from_matrices(self):
        """
        Test generating grain boundaries directly from transformation matrices.
        This is useful when the matrices are known or calculated externally.
        """
    ...
    def test_enum_sigma_cubic(self):
        """
        Test the enumeration of Sigma values for Cubic systems.
        Verifies that the calculated sigma values match expected theoretical values
        for various rotation axes ([100], [110], [111], etc.).
        """
    ...
    def test_enum_sigma_hex(self):
        """
        Test the enumeration of Sigma values for Hexagonal systems.
        """
    ...
    def test_enum_sigma_tet(self):
        """
        Test the enumeration of Sigma values for Tetragonal systems.
        """
    ...
    def test_enum_sigma_ort(self):
        """
        Test the enumeration of Sigma values for Orthorhombic systems.
        """
    ...
    def test_get_rotation_angle_from_sigma(self):
        """
        Test retrieving possible rotation angles for a given Sigma value.
        """
