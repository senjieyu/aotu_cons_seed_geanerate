...
class GBGenerator(object):
    """
    A class to generate grain boundaries (GBs) for various crystal systems.
    ...
    The generator supports two main modes of operation:
    1. From parameters: rotation axis, rotation angle (or sigma), plane, etc.
    2. From matrices: direct transformation matrices for the two grains.

    Key Features:
    - Supports both Twist and Tilt grain boundaries.
    - Can enumerate possible Sigma values and rotation angles for a given axis.
    - Handles vacuum thickness insertion.
    - Ensures non-interacting GBs by expanding the cell (expand_times).
    """
    ...
    def gb_from_parameters(self, rotation_axis, rotation_angle, expand_times=4, vacuum_thickness=0.0,
                           normal=False, ratio=None, plane=None, tol=1.e-4, shift=None, rm_tol=None):
        ...
        Args:
           ...
           vacuum_thickness (float): The thickness of vacuum that you want to insert between
            ...
            shift (list of floats):
                Displacement vector [da, db, dc] to shift the top grain relative to the bottom grain.
                Values are fractional coordinates relative to the GB supercell lattice vectors.
                Default is None (no shift).
            rm_tol (float):
                Distance tolerance for merging sites that are too close (e.g., at the interface).
                If None, no merging is performed. Default is None.
        Returns:
    ...
    def gb_from_matrices(self, top_grain_matrix, bottom_grain_matrix, expand_times, vacuum_thickness,
        ...
        whole_matrix_no_vac = half_lattice.matrix.copy()
        ...
        # Also apply the user-defined shift if provided
        cart_shift = np.array([0., 0., 0.])
        if shift is not None:
            # Convert fractional shift to Cartesian shift using the whole lattice
            cart_shift = whole_lat.get_cartesian_coords(shift)
        ...
        for site in top_grain:
            all_coords.append(site.coords + half_lattice.matrix[2] + translation_v + cart_shift)

        gb_with_vac = Structure(whole_lat, all_species, all_coords,
                                coords_are_cartesian=True,
                                site_properties={'grain_label': grain_labels})

        # Merge sites if rm_tol is specified
        if rm_tol is not None and rm_tol > 0:
            gb_with_vac.merge_sites(tol=rm_tol, mode='delete')
    ...
    def enum_sigma_cubic(cutoff, r_axis):
        ...
        # Algorithm based on Grimmer, Acta Cryst. A40, 108 (1984)
        # We iterate through possible integer values of n and m to find rotation angles
        # that yield a CSL (Coincidence Site Lattice) with a sigma value <= cutoff.
            ...
            for m in range(0, m_max + 1):
                if gcd(m, n) == 1 or m == 0:
                    ...
                    if odd_qua == 4:
                        ...
                        a = 1
                    # Calculate Sigma based on the quadruple [m, U, V, W]
                    # The formula depends on 'a' which is determined by the parity of the quadruple elements.
                    sigma = int(round((m ** 2 + n ** 2 * sum(np.array(r_axis) ** 2)) / a))
                    if (sigma <= cutoff) and (sigma > 1):
                        # Store unique rotation angles for each found Sigma
